...
#define USS_UUPS_STOPPED_BY_DEBUGGER                    BITF
 ...
 //!
 #define USS_TIMER_EXPIRED_EVENT1_INTERRUPT             (0x00020000)

 //! \brief This bit determines if the second event of period measurement is complete
 //!
 #define USS_TIMER_EXPIRED_EVENT2_INTERRUPT             (0x00040000)

 //! \brief This mask is used to clear all the USS interrupts, excluding timer related
 //!
#define USS_INTERRUPTS_MASK                              (0x0001DFFF)
...
typedef enum _USS_message_code_
{
	...
	//!< Value 0 - These message codes are use to provide status/error messages
	//!< to the user all APIs will return a message code after the execution.

	// 1-20 reserved for SAPH messages
	USS_message_code_SAPH_pulseLowPhasePeriod_error = 1,
	//!< Value 1 - This error is returned when the SAPH low phase period
	//!< configuration is invalid
	USS_message_code_SAPH_pulseHighPhasePeriod_error = 2,
	//!< Value 2 - This error is returned when the SAPH high phase period
	//!< configuration is invalid
	USS_message_code_SAPH_numOfExcitationPulses_error = 3,
	//!< Value 3 - This error is returned when SAPH number of excitation pulses
	//!< is invalid
	USS_message_code_SAPH_numOfStopPulses_error = 4,
	//!< Value 4 - This error is returned when SAPH number of stop pulses is
	//!< invalid
	USS_message_code_SAPH_update_error_ongoing_conversion = 5,
	//!< Value 5 - This error is returned if user attempted to update SAPH
	//!< during an active conversion  
	...
	//!< Value 12 - This error is returned if an invalid bias impedance value is
	//!< selected
    USS_message_code_SAPH_invalid_rx_charge_pump_mode_error=13,
    //!< Value 13 - This error is returned if an invalid charge pump configuration
    //!< is selected
    USS_message_code_SAPH_invalid_pulse_configuration_error=14,
    //!< Value 14 - This error is returned if an invalid pulse generation
    //!<  parameters are invalid. This error is returned in the following cases:
    //!< <ul>
	...
	//!< Value 26 - This error is returned if user attempted to update HSPLL during an
	//!< active conversion 
	USS_message_code_HSPLL_verification_expected_count_error=27,
	//!< Value 27 - This error is returned if USS_verifyHSPLLFrequency API if
	...
	//!< Value 41 - This error is returned if high and/or low threshold are invalid
	USS_message_code_SDHS_conversion_overflow_error = 43,
	//!< Value 43 - This error is returned if a conversion overflow is detected
	//!< during conversion
	...
	//!< Value 45 - This error is returned if user attempted to update SDHS
	//!< during anactive conversion 
	USS_message_code_SDHS_window_low_threshold_reached = 46,
	//!< Value 46 - This error is returned if conversion reached low threshold
	//!< during conversion
	USS_message_code_SDHS_window_high_threshold_reached = 47,
	//!< Value 47 - This error is returned if conversion reached high threshold
	//!< during conversion
	...
	//!< Value 61 - This error is returned if user attempted to update UUPS
	//!< during an active conversion
	...
	USS_message_code_measurement_period_overflow = 103,
	...
	//!< Value 121 - This error is returned if invalid parameters are used to
	//!< initialize algorithms
	USS_message_code_valid_results = 122,
	//!< Value 122 - This error is returned if NO problems where encountered
	//!< while running the algorithms
	USS_message_code_algorithm_error = 123,
	//!< Value 123 - This error is returned if problems were encountered while
	...
	//!< Value 124 - This error is returned the user has selected an invalid
	//!< number of absTOFInterval. The results return by USS_runAlgorithms or
	//!< USS_runAlgorithmsFixedPoint are invalid if this error is returned
	USS_message_code_algorithm_error_no_signal_detected_ups_channel = 125,
	//!< Value 125 - This error is returned if no signal is detected in UP stream channel.
	//!< The results return by USS_runAlgorihtms or USS_runAlgorithmsFixedPoint
	//!< are invalid if this error is returned.
	USS_message_code_algorithm_error_no_signal_detected_ups_dns_channel = 126,
	//!< Value 126 - This error is returned if no signal is detected in UP AND DOWN stream channel.
	//!< The results return by USS_runAlgorihtms or USS_runAlgorithmsFixedPoint
	//!< are invalid if this error is returned.
	USS_message_code_algorithm_error_no_signal_detected_dns_channel = 127,
	//!< Value 127 - This error is returned if no signal is detected in DOWN stream channel
	//!< The results return by USS_runAlgorihtms or USS_runAlgorithmsFixedPoint
	//!< are invalid if this error is returned.
	USS_message_code_algorithm_captures_accumulated = 128,
	//!< Value 128 - This message is returned by USS_runAlgorihtms or USS_runAlgorithmsFixedPoint
	//!< to indicate that UPS/DNS have only being accumulated
	USS_message_code_algorithm_error_invalid_clock_relative_error = 129,
	//!< Value 129 - This message is returned by USS_updateClockRelativeError to indicate
	//!< that clockRelativeError is outside tha valid range.
	USS_message_code_algorithm_error_invalid_filter_length = 130,
	//!< Value 130 - This message is returned by USS_initAlgorithms API if user
	...
	//!< Value 131 - This message is returned by USS_initAlgorithms API. This error is
	...
	//!< Value 132 - This message is returned by USS_initAlgorithms API if user
	//!< has enabled USS_diagnostics_mode_2. This error is returned if
	...
	//!< Value 175 - Indicates a timeout occurred during signal gain calibration
	//!< routine
	USS_message_code_Signal_Gain_Calibration_settling= 176,
	//!< Value 176 - Indicates signal gain calibration routine ended during signal
	//!< settling
	//!< loop
	...
	//!< Value 179 - Indicates a error has occurred during Abs ToF and DToF
	//!< calculation
	USS_message_code_Calibration_Offset_invalid_configuration = 180,
	//!< Value 180 - Indicated a test configuration has occured while running
	//!< AbsToF and DToF offset calculation
	USS_message_code_Calibration_VFR_pt_inside_quadrilateral = 181,
	//!< Value 181 - Indicates pt (flow,temp) found inside quadrilateral in VFR
	//!< flow temperature calibration
	USS_message_code_Calibration_VFR_pt_outside_quadrilateral = 182,
	//!< Value 182 - Indicates pt (flow,temp) is outside quadrilateral in VFR flow
	//!< temperature calibration

	// 240-255 error
	//!< USS general error codes
	USS_message_code_ACLK_settling_time_out = 240,
	//!< Value 240 - This error is return when the USS module attempted to configure
	...
	//!< Value 241 - This error is return when a silicon rev B or later
	//!< functionality is attempted to be used in rev A silicon (XMS material).
	USS_message_code_USSXT_oscillator_not_stable_error = 253,
	//!< Value 253 - This error is returned when the USSXT oscillator is not
	//!< stable after start-up time
	USS_message_code_USS_ongoing_active_conversion_error = 254,
	//!< Value 254 - This error is return when the USS module was attempted to
	//!< be updated while USS_ BUSY bit was set
	USS_message_code_error_occurred = 255
	//!< Value 255 - This is a generic error this is error that is returned if
...
//! \brief Contain meter constant for flow VFR calibration
//!
typedef struct _USS_Algorithms_Flow_Meter_Constant_
{
...
}USS_Algorithms_Flow_Meter_Constant;

//! \brief Contain Flow VFR calibration object
//!
typedef struct _USS_Alg_volume_Flow_Calib_Object_
{
    ...
    //!< Minimum numOfMeterCalibrationRanges = 1;
    //!< Maximum numOfMeterCalibrationRanges = maxNumOfMeterCalibrationRanges;
}USS_Alg_volume_Flow_Calib_Object;

//! \brief Contain quadrilateral meter constants for flow + temperature VFR
//! calibration
//!
typedef struct _USS_Algorithms_Quad_Meter_Constant_
{
    int32_t  iq16M0;
...
}USS_Algorithms_Quad_Meter_Constant;

//! \brief Contain quadrilateral point in flow + temperature VFR calibration
//!
typedef struct _USS_Algorithms_Quad_Point_
{
    int32_t  iq16Flow;
    //!< Specifies measured flow (resolution = 0.000 015 259)
    //!< Minimum flow = -32,768
    //!< Maximum flow = 32,767.999
    int32_t  iq16Temp;
    //!< specifies measured temperature (resolution = 0.000 015 259)
    //!< Minimum temp = -35
    //!< Maximum temp = 255.992
}USS_Algorithms_Quad_Point;

//! \brief Enumerated type for flow temperature VFR calibration state machine
//!
typedef enum _USS_Alg_volume_flow_Calculation_state_{
    USS_Alg_volume_flow_Calibration_state_track = 0,
    USS_Alg_volume_flow_Calibration_state_search = 1,
} USS_Alg_volume_flow_Calibration_state;

//! \brief Contain Flow Temperature VFR calibration object
//!
typedef struct _USS_Alg_volume_Flow_Temperature_Calib_Object_
{
    ...
    //!< Minimum numOfFlows = 2
    //!< Maximum numOfFlows = 255
    uint8_t numOfTemps;
    //!< Number of Temperature measurements
    //!< Minimum numOfTemps = 2
    //!< Maximum numOfTemps = 255
    uint8_t searchQuadCount;
    //!< Number of quadrilaterals searched in the previous calibration calculation
    //!< Minimum = 1
    //!< Maximum = 8
    uint8_t rowNum;
...
//! \brief Contain temperature lookup table data
//!
typedef struct _USS_Alg_temperature_Calculation_Object_
{
    const int32_t (*pTempLUT)[3];
    //!< Pointer to look-up table (LUT) used for temperature calculation.
    uint16_t tempLUTLength;
    //!< Length of look-up table (LUT) used for temperature calculation.
    //!< TODO: Check assumption customer's req resolution of .1, if we only 
    //!< support .5 at the least then switch data type to uint8_t
    //!< Also verify temperature will only be in Celcious
    int32_t iq24transducerConstant;
    //!< Transducer Length (L) / 2 used to calculate velocity
}USS_Alg_temperature_Calculation_Object;

//! \brief Enumerated type for temperature calculation
//!
typedef enum _USS_Alg_temperature_Calculation_Option_{
    USS_Alg_temperature_Calculation_Option_disabled = 0,
    USS_Alg_temperature_Calculation_Option_enabled = 1
} USS_Alg_temperature_Calculation_Option;

//! \brief Enumerated type for volume flow rate calibration algorithm selection.
//!
typedef enum _USS_Alg_volume_flow_Calibration_Option_{
    USS_Alg_volume_flow_Calibration_Option_disabled = 0,
    USS_Alg_volume_flow_Calibration_Option_flow = 1,
    USS_Alg_volume_flow_Calibration_Option_flow_temperature = 2,
}USS_Alg_volume_flow_Calibration_Option;
...
typedef struct _USS_Meter_Configuration_
{
	...
	//!< Volume Additional scale factor. 
	//!< This value is used if the volumeScaleFactor needs to be larger than 
	//!< the defined range.
...
typedef struct _USS_Capture_Configuration_
{
	...
	//!< \" <b>USS_CAPTURE_SEQUENCE_SELECTION_CH0_CH1</b> \" then a total of
	//!< <b>2</b>*sampleSize
	//!< SDHS samples will be captured at the end of the acquisition sequence.
	//!< If acquisition sequence selected is
	//!< \" <b>USS_CAPTURE_SEQUENCE_SELECTION_CH0_CH1_ch0</b> \" then a total
...
/**
 *  @brief Enumerated type for selection absTOF state.
 */
typedef enum _USS_absTOF_state_t_
{
    /** Executes full search */
    absTOF_state_search = 0,
    /** Executes track state based on iq16Index. */
    absTOF_state_track = 1,
    /** Only calculates total TOF using iq16Index. */
    absTOF_state_bypass = 2,
} USS_absTOF_state_t;

/**
 *  @brief Structure containing absTOF channel status object.
 */
typedef struct _USS_absTOF_channel_status_object_ {
    /** Sample Index of MAX of Signal */
    int16_t indexMaxAmplitude;
    /** Index of absTOF channel. (16 bits integer, 16 bits fractional)  */
    int32_t iq16Index;
    /** State of the absTOF algorithm*/
    USS_absTOF_state_t state;
} USS_absTOF_channel_status_object;

/**
 *  @brief Structure containing absTOF status object.
 */
typedef struct _USS_absTOF_status_object_ {
    USS_absTOF_channel_status_object UPS;
    USS_absTOF_channel_status_object DNS;
} USS_absTOF_status_object;
...
typedef struct _USS_Algorithms_User_Configuration_
{
    ...
    //!< This variable is used to define which windowing configuration is enabled .
    //!< When windowing is enabled, it allows you to specify the window type to be used.
    //!< This window is only applied on the captured DNS & UPS signals for the
    //!< Delta ToF (DToF) computation. The options below are only applicable
    //!< when using USS_ALG_DTOF_COMPUTATION_MODE= USS_ALG_DTOF_COMPUTATION_OPTION_ESTIMATE.
    //!< The memory (in bytes) required when windowing is not USS_ALG_DTOF_EST_WINDOW_OPTION_DISABLED
    //!< can be calculated using: USS_SW_LIB_APP_MAX_WINDOW_RAMP_SIZE  =
    //!< 2 * [ROUND ( USS_ALG_WIN_TRAP_RAMP_NUM_CYCLES *
    //!< [(USS_PLL_FREQ / USS_SDHS_OVER_SAMPLE_RATE) / USS_F1_FREQ ] ) + 2];
    //!<
    //!<  USS_ALG_DTOF_EST_WINDOW_OPTION_DISABLED : This setting disables the
    //!< window application on the captured signal.
    //!<
    //!< USS_ALG_DTOF_EST_WINDOW_OPTION_STATIC: The applied window is static with
    //!< reference to the start of the capture signal. Even if the captured DNS
    //!< and UPS signals move because of changes in flow rates or due to temperature,
    //!< the applied window is always the same with reference to the start of the
    //!< capture signal.
    //!< The applied window is trapezoidal in nature and specified by
    //!< .winTrapzRampOverNumCycles (USS_ALG_WIN_TRAP_RAMP_NUM_CYCLES),
    //!< .winNumCycles (USS_ALG_WIN_NUM_CYCLES) and .winStartIndexBackOffNumCycles
    //!< (USS_ALG_WIN_START_INDEX_BACK_OFF_NUM_CYCLES). In all these parameters,
    //!< the NumCycles refers to the number of transducer cycles.
    //!< winStartIndexBackOffNumCycles defines the window start with reference
    //!< to the start of the capture signal. The window is 0 from the start of the
    //!< capture to winStartIndexBackOffNumCycles. winTrapzRampOverNumCycles defines
    //!< the number of cycles over which the window ramps from 0 to 1. It similarly
    //!< defines the number of cycles over which the window ramps down from 1 to 0.
    //!< winNumCycles defines the length of the window in terms of transducer cycles
    //!< when it is 1. In effect, the total window length is defined by
    //!< winTrapzRampOverNumCycles + winNumCycles + winTrapzRampOverNumCycles and
    //!< the position is defined by winStartIndexBackOffNumCycles.
    //!<
    //!< USS_ALG_DTOF_EST_WINDOW_OPTION_ESTIMATE: The applied window is static in
    //!< terms of the length but the start position of the applied window is
    //!< dependent on the start of the signal defined by AbsToF for UPS and DNS
    //!< independently. The applied window shape is similar to the window defined
    //!< in USS_ALG_DTOF_EST_WINDOW_OPTION_STATIC.
    //!<
    //!< USS_ALG_DTOF_EST_WINDOW_OPTION_DYNAMIC: The applied window is dynamic in
    //!< terms of the size of the window itself and is defined with reference to
    //!< the position of the peak of the signal. The applied window is trapezoidal
    //!< in nature and specified by .winTrapzRampOverNumCycles (USS_ALG_WIN_TRAP_RAMP_NUM_CYCLES),
    //!< .winStartIndexBackOffNumCycles (USS_ALG_WIN_START_INDEX_BACK_OFF_NUM_CYCLES)
    //!< and .winPeakIndex2EdgeNumCycles (USS_ALG_WIN_PEAK_INDEX_2_EDGES_NUM_CYLES).
    //!< winStartIndexBackOffNumCycles defines the window start with reference to
    //!< the start of the capture signal. winTrapzRampOverNumCycles defines the
    //!< number of cycles over which the window ramps from 0 to 1. It similarly
    //!< defines the number of cycles over which the window ramps down from 1 to
    //!< 0. The window is set to 1 till winPeakIndex2EdgeNumCycles after the peak
    //!< of the signal. In effect, the total window length is not known and is
    //!< dependent on the position of the peak of the signal.
    //!<
    //!< If USS_ALG_DTOF_COMPUTATION_MODE= USS_ALG_DTOF_COMPUTATION_OPTION_WATER,
    //!< then the windowingMode is a flag to enable or disable windowing. Memory
    //!< in bytes required for windowing is
    //!< USS_SW_LIB_APP_MAX_WINDOW_RAMP_SIZE = 2 * (USS_SW_LIB_APP_MAX_CAPTURE_SIZE +
    //!< USS_SW_LIB_APP_MAX_FILTER_LENGTH)
    //!< The options below are only applicable
    //!< when using USS_ALG_DTOF_COMPUTATION_MODE= USS_ALG_DTOF_COMPUTATION_OPTION_WATER.
    //!<
    //!< USS_ALG_DTOF_WATER_WINDOW_OPTION_DISABLED: This setting disables the
    //!< window application on the captured signal
    //!<
    //!< USS_ALG_DTOF_WATER_WINDOW_OPTION_ENABLE: The applied window is static
    //!< in terms of the length but the start position of the applied window is
    //!< dependent on the start of the signal defined by AbsToF for DNS. The same
    //!< start position is also used for UPS.  The applied window shape is similar
    //!< to the window defined in USS_ALG_DTOF_EST_WINDOW_OPTION_STATIC.
    ...
    //!< (derived from absTOF) (applies to static windowing only)
    ...
    //!< The number of cycles from peak of signal to edge (or start) of window down ramp
    //!< (applies to dynamic windowing only)
    ...
    //!< For DToF Gas SingleTone Correlated: winLength is Dynamic internally and set as
    //!< winLength = numSamples2Peak + (numSamplesPerCycle * winPeakIndex2EdgeNumCycles)
    //!< + (winRampLength)
    //!< Where,
    //!< numSamples2Peak = Dynamically estimated each measurement as Peak of signal
    //!< winNumCycles is ignored in this mode
    bool enableFilter;
    ...
    union {
        ...
        //!< Specifies Volume Flow Rate Flow (only) Calibration object,
        //!< it must be initialized for when volumeCalibrationOption =
        //!< USS_Alg_volume_flow_Calibration_Option_flow
        USS_Alg_volume_Flow_Temperature_Calib_Object * flowTempCalibObject;
        //!< Specifies Volume Flow Rate Flow + Temperature Calibration object,
        //!< it must be initialized for when volumeCalibrationOption =
        //!< USS_Alg_volume_flow_Calibration_Option_flow_temperature
    } calibObject;
    USS_Alg_temperature_Calculation_Object * tempCalcObject;
    //!< Specifies Temperature Calculation object used to calculate temperature
    //!< using UPS and DNS absToF.
...
//! \brief The following variable is used to reload the measurement timer period
extern volatile uint16_t USSSWLIB_USS_measurementPeriod;
...
//!	 USS_CAPTURE_SEQUENCE_SELECTION_CH0_CH1 capture mode.
...
//! \brief    The following API can be used to set the application timer period
//!        between USS measurements. The API allows to select the desired Low
//!        Power Mode. Global interrupts remain enable. The timer clock source
//!        for this background delay is ACLK. Therefore the timer period will
//!        be determined based on ACLK frequency (e.g. for count = 30 and ACLK
//!        freq = 32768 HZ a timer period of ~915 msec will be generated.
//!        This API will configure the config->systemConfig->measurementPeriod.
//!
//!
//! \param[in] config  is a pointer to the USS SW Library user configuration
//! \param[in] count Determines the background timer period.
//!  Minimum = 655 (20 milliseconds)
//!  Maximum = 65535 (2 seconds)
//!
//! \return none
//!
extern void USS_configAppTimerPeriod(
        USS_SW_Library_configuration *config,
        uint16_t count);

//! \brief    The following API can be used to wait until the background timer
//!        between USS measurements has elapsed. If the timer has elapsed prior to
//!        entering the API it will return USS_message_code_measurement_period_overflow,
//!        otherwise it will wait until the timer elapses, then return
//!        USS_message_code_no_error.
//! \n \n \b Note1: USS_configAppTimerPeriod must be called prior to calling this
//! function.
//!
//! \param[in] config  is a pointer to the USS SW Library user configuration
//! \param[in] lpmMode selects the low power mode Possible values are:
...
//! \return USS_message_code
//!
extern USS_message_code USS_waitForAppTimerElapse(
        USS_SW_Library_configuration *config,
        USS_low_power_mode_option lpmMode);
...
//! For example: if sequenceSelection == USS_CAPTURE_SEQUENCE_SELECTION_CH0_CH1
...
//! \brief The following API calibrates Volume Flow Rate using fixed point math.
//!
//! \n \n \b Note1: USS_runAlgorithmsFixedPoint must be called prior to calling
//! this function.
//!
//! The API checks the volumeCalibrationOption member in the
//! USS_Algorithms_User_Configuration structure.
//! For example:
//! If volumeCalibrationOption == USS_Alg_volume_flow_Calibration_Option_flow
//! Then this API executes USS_calibrateVFRFlow
//! If volumeCalibrationOption == USS_Alg_volume_flow_Calibration_Option_flow_temperature
//! Then this API executes USS_calibrateVFRFlowTemperature
//! If API return message code USS_message_code_valid_results, then algorithms
//! result are valid.
//!
//! \param[in] config  is a pointer to the USS SW Library user configuration
...
//!
//! \return USS_message_code
extern USS_message_code USS_calibrateVolumeFlowRate(
        USS_SW_Library_configuration *config,
        USS_Algorithms_Results_fixed_point *results);

//! \brief The following API calibrates Volume Flow Rate using flow (only).
//!
//! \n \n \b Note1: USS_runAlgorithmsFixedPoint must be called prior to calling
//! this function.
//!
//! The API checks the meter constant Look-Up-Table (LUT) using the measured
//! volume flow rate. The calibrated volume flow rate is stored in the
//! results->iq16VolumeFlowRate member, and the un calibrated result is
//! stored in results->iq16VolumeFlowRateNoCalib member. For measured
//! volume flow rate below the minimum VFR defined in the LUT,
//! the minimum VFR corresponding meter constant will be used.
//! If API return message code USS_message_code_valid_results, then algorithms
//! result are valid.
//!
//! \param[in] config  is a pointer to the USS SW Library user configuration
...
//!
//! \return USS_message_code
extern USS_message_code USS_calibrateVFRFlow(
        USS_SW_Library_configuration *config,
        USS_Algorithms_Results_fixed_point *results);

//! \brief The following API calibrates Volume Flow Rate using flow and temperature
//!
//! \n \n \b Note1: USS_runAlgorithmsFixedPoint must be called prior to calling
//! this function.
//!
//! The API uses the measured volume flow rate and temperature to find 
//! the corresponding quadrilateral. The calibration algorithm has 2 states, 
//! a search and track state. The first iteration it will run a full search 
//! of the quadrilateral using column (flow) first, then search via row (temperature). 
//! If a quadrilateral is found the next iteration of the algorithm will be in
//! track mode, searching the previously found quadrilateral. If the flow,
//! temperature pt is outside, then it will search the surrounding quadrilaterals.
//! If the search is unsuccesfull, it will go back to search mode. The calibrated 
//! volume flow rate is stored in the results->iq16VolumeFlowRate member, and 
//! the un calibrated result is stored in results->iq16VolumeFlowRateNoCalib member. 
//! For measured volume flow rate, temperature outside of the defined quadrilaterals, 
//! the closest qudrilateral meter constants will be used for calibration. 
//! If API return message code USS_message_code_valid_results, then algorithms
//! result are valid.
//!
//! \param[in] config  is a pointer to the USS SW Library user configuration
...
//!
//! \return USS_message_code
extern USS_message_code USS_calibrateVFRFlowTemperature(
        USS_SW_Library_configuration *config,
        USS_Algorithms_Results_fixed_point *results);
...
//! \brief The following API computes temperature using UPS and DNS AbsToF
//!
//! \n \n \b Note1: USS_runAlgorithmsFixedPoint must be called prior to calling
//! this function.
//!
//! The API calculates the sound velocity (c) using UPS and DNS based on the formula:
//! c = (L/2)*( (1/UPS) + (1/DNS)) = (L/2)*( (UPS+DNS)/(UPS*DNS) )
//! Then it converts the velocity to temperature using the temperature 
//! Look-Up-Table (LUT).
//! When the velocity calculated is outside of the LUT range.
//! If velocity is below the lowest velocity in the LUT,
//!    then the lowest 2 LUT values will be used to calculate temperature
//! If velocity is above the highest velocity in the LUT,
//!    then the highest 2 LUT values will be used to calculate temperature
//!
//! \param[in] config  is a pointer to the USS SW Library user configuration
//! \param[in] results  is a pointer to the USS Algorithms Result structure
//!
//! \return USS_message_code
extern USS_message_code USS_computeTemperature(
        USS_SW_Library_configuration *config,
        USS_Algorithms_Results_fixed_point *results);

//! \brief The following API updates the fixed point temperature
//!
//! \n \n \b Note1: USS_runAlgorithmsFixedPoint must be called prior to calling
//! this function.
//!
//! The API converts the temperature parameter to IQ16 format, the result is 
//! stored in results->iq16Temperature. This API must be used if 
//! volumeCalibrationOption == USS_Alg_volume_flow_Calibration_Option_flow_temperature
//! and temperatureCalcOption == USS_Alg_temperature_Calculation_Option_disabled;
//! for applications where the temperature is calculated using an external sensor. 
//!
//! \param[in] temperature  is the new temperature value
//! \param[in] results  is a pointer to the USS Algorithms Result structure
//!
//! \return USS_message_code
extern USS_message_code USS_setTemperature(
        float temperature,
        USS_Algorithms_Results_fixed_point *results);
...
//! \brief This function allows users to update the internal AbsTOF status.
//! Including the state and index for the UPS / DNS channels.
//!
//! \param[in] config  is a pointer to the USS SW Library user configuration.
//! \param[in] status  is a pointer to the USS absTOF status object
//!
//! \return USS_message_code
extern USS_message_code USS_setAbsTOFStatus (USS_SW_Library_configuration *config,
                                             USS_absTOF_status_object *status);

//! \brief This function allows users to obtain the internal AbsTOF status.
//! Including the state and index for the UPS / DNS channels.
//!
//! \param[in] config  is a pointer to the USS SW Library user configuration.
//! \param[in] status  is a pointer to the USS absTOF status object
//!
//! \return USS_message_code
extern USS_message_code USS_getAbsTOFStatus (USS_SW_Library_configuration *config,
                                             USS_absTOF_status_object *status);
...
//! \brief The following API allows to obtain the address where the library
//!        algorithm object is stored. The length of the object can be
//!        accessed using the USS_getAlgorithmObjLength API.
//!
//!
//! \return address to the first byte of the algorithm object
extern uint8_t* USS_getAlgorithmObjPtr(void);

//! \brief The following API allows to obtain the length of the library
//!        algorithm object.
//!
//!
//! \return length of the library algorithm object
extern uint16_t USS_getAlgorithmObjLength(void);
