...
#ifndef USSLIBGUIAPP_FR604X_USS_APP_RESULTSDC_RESULTS_H_
#define USSLIBGUIAPP_FR604X_USS_APP_RESULTSDC_RESULTS_H_

// number of measurements the statistics will be calculated over
#define X_NUM_MEASUREMENTS 5


// the Results data structure to be updated after each measurement while the LCD
// State machine is functioning
typedef struct
{
	float last_dtof;
	float last_X_dtofMeasurements[X_NUM_MEASUREMENTS];
	float avg_dtof;
	float stdev_dtof;

	float last_FlowRate;
	float last_X_FlowRateMeasurements[X_NUM_MEASUREMENTS];
	float avg_FlowRate;
	...
	uint16_t BatteryPercentageStamp;

	float temp;

	int DateAndTimeStamp;
}RESULTS_MEASUREMENT_DATA;

extern RESULTS_MEASUREMENT_DATA g_ResultsOfLastMeasurement;

//! \brief This routine resets the results data structure for the LCD state
//!        machine with the proper initial data values
//!
//! \return none
void results_Reset(bool unit);

//! \brief This routine updates the results data structure for the LCD state
//!        machine with the new results data from the USS measurement
//!
//! \return none
void results_Update(float dtofData, float volrateData, uint16_t rate);

//! \brief This routine calculates the proper volume data based upon the unit
//!        indicated in the GUI
//!
//! \return float Volume
float resultsCalcVolume(uint16_t rate);

#endif /* USSLIBGUIAPP_FR604X_USS_APP_RESULTSDC_RESULTS_H_ */
