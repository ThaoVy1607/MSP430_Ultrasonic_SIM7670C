...
static __persistent DISPLAY_DATA_t displayData =
	{
		...
		.digit[0] = HAL_LCD_DIGIT_1,
		.digit[1] = HAL_LCD_DIGIT_2,
		.digit[2] = HAL_LCD_DIGIT_3,
		.digit[3] = HAL_LCD_DIGIT_4,
		.digit[4] = HAL_LCD_DIGIT_5,
		.digit[5] = HAL_LCD_DIGIT_6,
		.digit[6] = HAL_LCD_DIGIT_7,
		.digit[7] = HAL_LCD_DIGIT_8,
		.digit[8] = HAL_LCD_DIGIT_9,
		.digit[9] = HAL_LCD_DIGIT_10,
...
void lcd_statemachine_stateUpdate(void)
{
	...
	case HAL_SYS_BUTTON_LEFT:
		...
		break;
	case HAL_SYS_BUTTON_UP:
		if (g_lcd_statemachine.current_state == LCD_STATEMACHINE_flowRateAverage)
			g_lcd_statemachine.next_state = LCD_STATEMACHINE_sleepLCD;
		else
			g_lcd_statemachine.next_state = LCD_STATEMACHINE_flowRateAverage;
...
#ifdef DELTATOF
		if (LCD_STATEMACHINE_deltaTOF == *state)
		{
			success = true;
		}
#endif
#ifdef DELTATOFAVERAGE
		if (LCD_STATEMACHINE_deltaTOFAverage == *state)
		{
			success = true;
		}
#endif
#ifdef DELTATOFSTANDARDDEVIATION
		if (LCD_STATEMACHINE_deltaTOFstandardDeviation == *state)
		{
			success = true;
		}
#endif
#ifdef FLOWRATE
		if (LCD_STATEMACHINE_flowRate == *state)
		{
			success = true;
		}
#endif
#ifdef FLOWRATEAVERAGE
		if (LCD_STATEMACHINE_flowRateAverage == *state)
		{
			success = true;
		}
#endif
#ifdef VOLUME
		if (LCD_STATEMACHINE_volume == *state)
		{
			success = true;
		}
#endif
#ifdef DATEANDTIME
		if (LCD_STATEMACHINE_dateAndTime == *state)
		{
			success = true;
		}
#endif
#ifdef TEMPURATURE
		if (LCD_STATEMACHINE_temperature == *state)
		{
			success = true;
		}
#endif
#ifdef BATTERYVOLTAGE
		if (LCD_STATEMACHINE_batteryVoltage == *state)
		{
			success = true;
		}
#endif
