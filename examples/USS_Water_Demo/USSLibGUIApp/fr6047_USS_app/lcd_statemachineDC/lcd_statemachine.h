...
#ifndef LCD_STATEMACHINEDC_LCD_STATEMACHINE_H_
#define LCD_STATEMACHINEDC_LCD_STATEMACHINE_H_
...
// max captures for scroll state for each statistics
#define MAX_SCROLL_ITERATIONS 	25
// resolution of the LCD data
#define RESOLUTION		6

#define RIGHT	true
#define LEFT	false

// Enable or Disable the statistics with the #defines below

#define DELTATOF
#define DELTATOFAVERAGE
#define DELTATOFSTANDARDDEVIATION
#define FLOWRATE
#define FLOWRATEAVERAGE
#define VOLUME
//#define DATEANDTIME
#ifdef ENABLE_ADC
//#define TEMPERATURE
...
// all the different states of the LCD state machine
typedef enum {
	LCD_STATEMACHINE_ZERO,
	LCD_STATEMACHINE_ONE,
	LCD_STATEMACHINE_sleepLCD,
	LCD_STATEMACHINE_mainMenuScroll,
	LCD_STATEMACHINE_minStates,
	LCD_STATEMACHINE_deltaTOF,
	LCD_STATEMACHINE_deltaTOFAverage,
	LCD_STATEMACHINE_deltaTOFstandardDeviation,
	LCD_STATEMACHINE_flowRate,
	LCD_STATEMACHINE_flowRateAverage,
	LCD_STATEMACHINE_volume,
	LCD_STATEMACHINE_dateAndTime,
	LCD_STATEMACHINE_temperature,
	LCD_STATEMACHINE_batteryVoltage,
	LCD_STATEMACHINE_maxStates,
} LCD_STATEMACHINE_STATE;

// LCD state machines status struct
typedef struct
{
	LCD_STATEMACHINE_STATE previous_state;
	LCD_STATEMACHINE_STATE current_state;
	LCD_STATEMACHINE_STATE next_state;
	bool isfirsttime;
	bool isfirsttimescroll;

	LCD_STATEMACHINE_STATE scrollState;
	uint8_t scrollIteration;
...
// pointer to displayData struct variable
static DISPLAY_DATA_t * displayData_ptr;

//! \brief This routine updates the state of the LCD based upon the input from
//!         the buttons
//!
//! \return none
void lcd_statemachine_stateUpdate(void);

//! \brief This routine switches and calls the proper action based upon the
//!         state of the LCD
//!
//! \return none
void lcd_statemachine_stateAction(void);

//! \brief This routine clears the displayData struct
//!
//! \return bool true if it is asleep false if not
bool lcd_statemachine_isSleep(void);

//! \brief This routine scrolls the state of the LCD left or right through the
//!         different posible enabled statistics
//!
//! \return none
void lcd_satemachine_srollState(bool direction, LCD_STATEMACHINE_STATE * state );

//! \brief This routine writes the information in the  displayData struct
//!         to the lcd
//!
//! \return none
...
//! \brief This routine makes the LCD go to sleep and disables the results
//!         update functions
//!
//! \return none
void lcd_statemachine_sleepLCD(void);

//! \brief This routine scolls through displaying all of the enabled statistics
//!
//! \return none
void lcd_statemachine_displayScrollStatistics(void);

//! \brief This routine displays the  Delta Time of Flight in seconds
//!
//! \return none
void lcd_statemachine_displayDeltaTOF(void);

//! \brief This routine displays the Average  Delta Time of Flight in seconds
//!
//! \return none
void lcd_statemachine_displayAvgDeltaTOF(void);

//! \brief This routine displays the Standard Deviation of the
//!         Delta Time of Flight in seconds
//!
//! \return none
void lcd_statemachine_displayDeltaTOFStandardDeviation(void);

//! \brief This routine displays the Volume Flow Rate
//!         in Liter per hour or Gallons per minute
//!
//! \return none
void lcd_statemachine_displayFlowRate(void);

//! \brief This routine displays the Average Volume Flow Rate
//!         in Liter per hour or Gallons per minute
//!
//! \return none
void lcd_statemachine_displayAvgFlowRate(void);

//! \brief This routine displays the Volume in Liter or Gallons
//!
//! \return none
void lcd_statemachine_displayVolume(void);

//! \brief This routine displays the Date and Time
//!
//! \return none
void lcd_statemachine_displayDateAndTime(void);

//! \brief This routine displays the Temperature
//!
//! \return none
void lcd_statemachine_displayTemperature(void);

//! \brief This routine displays the Battery Voltage
//!
//! \return none
...
#endif /* LCD_STATEMACHINEDC_LCD_STATEMACHINE_H_ */
