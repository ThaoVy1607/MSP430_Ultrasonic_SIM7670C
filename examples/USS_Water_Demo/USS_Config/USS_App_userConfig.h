 ...
 *
 * --/COPYRIGHT--*/
#ifndef _USS_APP_USERCONFIG_H_
#define _USS_APP_USERCONFIG_H_

//#############################################################################
//
//! \file   USS_App_userConfig.h
//!
//! \brief  Configuration of Ultrasonic Application
//!         
//
//  Group:          MSP
...
//#############################################################################
// TI Release: UltrasonicWaterFR604x_02_40_00_00 
...
// Definitions
//*****************************************************************************
/* Enables Periodic resonator Calibration */
#ifdef DEBUG_CLOCK_CORRECTION
#define USS_APP_RESONATOR_CALIBRATE_ENABLE
#endif
/* Enables Periodic AGC Calibration */
//#define USS_APP_AGC_CALIBRATE_ENABLE
/* Enables DC Offset estimation and cancellation */
//#define USS_APP_DC_OFFSET_CANCELLATION_ENABLE
#if defined(USS_GAS_CONFIGURATION)
/* Enables Additional TX Sequence */
#define USS_APP_ADDTL_TX_SEQUENCE_ENABLE
#else
/* Disables additional TX Sequence */
#undef USS_APP_ADDTL_TX_SEQUENCE_ENABLE
#endif
...
#define APPLICATION_ABSTOF_DTOF_OFFSET_UPDATE_INTERVAL      256
#define APPLICATION_ABSTOF_DTOF_OFFSET_LPM_CAPTURE          true
#define APPLICATION_ABSTOF_DTOF_OFFSET_CALC_UPS_ABSTOF      true
#define APPLICATION_ABSTOF_DTOF_OFFSET_CALC_DNS_ABSTOF      true
#define APPLICATION_ABSTOF_DTOF_OFFSET_CALC_DTOF            true
...
/*! Defines the Interval for calibration of the resonator */
...
/*! Enables use of additional TX pattern: 0 disabled, 1+ enabled with
 * different configurations */
#define USS_APP_ADDTL_TX_SEQUENCE_MODE              (0)
/*! Defines the maximum number of TX patterns supported by application */
#define USS_APP_ADDTL_TX_SEQUENCES_MAX              (1)
/*! Defines the maximum number of Trill pulses of additional TX patterns */
#define USS_APP_ADDTL_TX_SEQUENCE_MAX_TRILL         (2)
/*! Defines the maximum number of Trill pulses of additional TX patterns */
#define USS_APP_ADDTL_TX_SEQUENCE_BINPATTERN_MAX    (25)
//*****************************************************************************
// typedefs
//*****************************************************************************
typedef struct
{
    uint16_t u16CaptureDelayACLK;
#ifdef USS_APP_RESONATOR_CALIBRATE_ENABLE
    uint16_t u16ResonatorCalibrateInterval;
#endif
#ifdef USS_APP_AGC_CALIBRATE_ENABLE
    uint16_t u16AGCCalibrateInterval;
#endif
#ifdef USS_APP_DC_OFFSET_CANCELLATION_ENABLE
    uint16_t u16DCOffsetEstimateInterval;
#endif
#ifdef USS_APP_ADDTL_TX_SEQUENCE_ENABLE
    uint16_t u16AddtTXSequenceMode;
#endif
}USS_App_userConfig_t;


#ifdef USS_APP_ADDTL_TX_SEQUENCE_ENABLE
/* Structure to support Additional TX sequences in demo application */
typedef struct
{
    /*! Number of Trill Cycles, each cycle defines 'n' x-pulses followed
     * by 'm' e-pulses.
     * The frequency of each pulse is defined by PLL / (hper + lper)
     * */
    uint16_t    numTrillCycles;
    /*! Number of x-pulses for each trill cycle */
    uint16_t    xpulse[USS_APP_ADDTL_TX_SEQUENCE_MAX_TRILL];
    /*! High period for x-pulses for each trill cycle */
    uint16_t    xhper[USS_APP_ADDTL_TX_SEQUENCE_MAX_TRILL];
    /*! Low period for x-pulses for each trill cycle */
    uint16_t    xlper[USS_APP_ADDTL_TX_SEQUENCE_MAX_TRILL];
    /*! Number of e-pulses for each trill cycle */
    uint16_t    epulse[USS_APP_ADDTL_TX_SEQUENCE_MAX_TRILL];
    /*! High period for e-pulses for each trill cycle */
    uint16_t    ehper[USS_APP_ADDTL_TX_SEQUENCE_MAX_TRILL];
    /*! Low period for e-pulses for each trill cycle */
    uint16_t    elper[USS_APP_ADDTL_TX_SEQUENCE_MAX_TRILL];
    /*! Size of the binary pattern */
    uint16_t    binPatternLen;
    /*! Binary pattern */
    int16_t     binPattern[USS_APP_ADDTL_TX_SEQUENCE_BINPATTERN_MAX];
}USS_App_AddlTXSequence_t;
#endif

extern USS_App_userConfig_t USS_App_userConfig;

#endif // end of  _USS_APP_USERCONFIG_H_
