...
void hal_system_ButtonsInit(void)
{
    ...
    GPIO_selectInterruptEdge(HAL_SYS_BUTTON_SEL_PORT,
                             HAL_SYS_BUTTON_SEL_PIN,
                             GPIO_HIGH_TO_LOW_TRANSITION);
    GPIO_selectInterruptEdge(HAL_SYS_BUTTON_RIGHT_PORT,
...
uint16_t hal_system_ButtonsCheck(void)
{
    ...
    if (!GPIO_getInputPinValue(HAL_SYS_BUTTON_LEFT_PORT, HAL_SYS_BUTTON_LEFT_PIN))
    {
        ret_val |= HAL_SYS_BUTTON_LEFT;
    }

    if (!GPIO_getInputPinValue(HAL_SYS_BUTTON_SEL_PORT, HAL_SYS_BUTTON_SEL_PIN))
    {
        ret_val |= HAL_SYS_BUTTON_SEL;
    }

    if (!GPIO_getInputPinValue(HAL_SYS_BUTTON_RIGHT_PORT, HAL_SYS_BUTTON_RIGHT_PIN))
    {
...
static void hal_system_GPIOInit(void)
{
     ...
     *  P1[4] IRQ (OPEN DRAIN)
     *  P1[5] BUT_SEL (input with internal pull-up)
     *  P1[6:7]  P1.6: UCB0 I2C SDA & P1.7: UCB0 I2C SCL
     *  P3[1] BUT_LEFT (input with internal pull-up)
     *  P4[0] BUT_RIGHT (input with internal pull-up)
     *  PJ[4:5] LFXT
     */
    ...
    GPIO_setOutputLowOnPin(GPIO_PORT_P3,
                           GPIO_PIN0|GPIO_PIN2|GPIO_PIN3|GPIO_PIN4|GPIO_PIN5|GPIO_PIN6|GPIO_PIN7);
